    ...
    def draw(self):
        ...
        for i in range(self.numX):
            for j in range(self.numY):
                ...
                pygame.draw.rect(WIN, ((self.density, self.density, self.density)), (x, y, size, size), 1)
    ...
    def calcDivergence(self):
        for i in range(self.numX):
            for j in range(self.numY):
                print (i, j)
                print (self.vel[0, 20])
                self.divergence[i, j] = ((self.vel[i+1, j, 0] - self.vel[i, j, 0]) + (self.vel[i, j+1, 1] - self.vel[i, j, 1]))*overrelaxation
