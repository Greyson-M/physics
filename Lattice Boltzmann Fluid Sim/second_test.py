        ...
        for k in range(self.iter):
            for i in range(1, self.size - 1):
                for j in range(1, self.size - 1):
                    ...
                    x[i][j] = (x0[i][j] + a * (x[i + 1][j] + x[i - 1][j] + x[i][j + 1] + x[i][j - 1])) * recip_c
    ...
    def set_bnd(self, b, x):
        for i in range(1, self.size - 1):
           ...
           # x[self.IX(i, self.size - 1)] = -x[self.IX(i, self.size - 2)] if b == 2 else x[self.IX(i, self.size - 2)]
            ...
            x[i][self.size - 1] = -x[i][self.size - 2] if b == 2 else x[i][self.size - 2]
        ...
        for j in range(1, self.size - 1):
            ...
            x[self.size - 1][j] = -x[self.size - 2][j] if b == 1 else x[self.size - 2][j]
    ...
    def project(self, velocX, velocY, p, div):
        for j in range(1, self.size - 1):
            for i in range(1, self.size - 1):
                ...
                #p[self.IX(i, j)] = 0
                div[i][j] = -0.5 * (velocX[i + 1][j] - velocX[i - 1][j] + velocY[i][j + 1] - velocY[i][j - 1]) / self.size
                p[i][j] = 0
